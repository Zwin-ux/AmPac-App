from fastapi import APIRouter, HTTPException, Header, Depends
from typing import List, Optional
from app.schemas.application import ApplicationCreate, ApplicationUpdate, ApplicationResponse
from app.services.application_service import application_service

router = APIRouter()

@router.get("/ping")
async def ping():
    return {"status": "pong", "service": "applications"}

@router.get("/user/me", response_model=List[ApplicationResponse])
async def list_my_applications(x_user_id: Optional[str] = Header(None)):
    """
    List all applications for the current user.
    """
    if not x_user_id:
        # Fallback for demo if header missing, though in real app this would be 401
        return []
        
    return await application_service.list_user_applications(x_user_id)

@router.post("/", response_model=ApplicationResponse)
async def create_application(request: ApplicationCreate):
    """
    Create a new loan application.
    """
    try:
        return await application_service.create_application(request)
    except Exception as e:
        raise HTTPException(status_code=500, detail=str(e))

@router.get("/{app_id}", response_model=ApplicationResponse)
async def get_application(app_id: str):
    """
    Get application details by ID.
    """
    app = await application_service.get_application(app_id)
    if not app:
        raise HTTPException(status_code=404, detail="Application not found")
    return app

@router.put("/{app_id}", response_model=ApplicationResponse)
async def update_application(app_id: str, request: ApplicationUpdate):
    """
    Update an existing application (autosave).
    """
    app = await application_service.update_application(app_id, request)
    if not app:
        raise HTTPException(status_code=404, detail="Application not found")
    return app

@router.post("/{app_id}/submit", response_model=ApplicationResponse)
async def submit_application(app_id: str):
    """
    Submit an application for review.
    """
    app = await application_service.submit_application(app_id)
    if not app:
        raise HTTPException(status_code=404, detail="Application not found")
    return app
